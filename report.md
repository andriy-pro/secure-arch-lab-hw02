...
- Призначати роль тільки на сервері за безпечним дефолтом, наприклад `customer`, а зміну ролей робити через окремий адміністративний workflow з аудитом.
...
Перевірка підтвердила 5 практичних прикладів REST API проблем: неправильну авторизацію функції, IDOR/BOLA, SQL Injection, Mass Assignment і зайве розкриття даних. Найважливіші напрями виправлення: централізована авторизація, перевірка ownership для об'єктів, параметризовані запити, whitelist/DTO для вхідних полів і обмеження полів у відповідях API.
